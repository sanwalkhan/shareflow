...
const dummyPayrollData = [
  { id: "1", name: "John Doe", amount: "$5000", date: "2025-12-01" },
  { id: "2", name: "Jane Smith", amount: "$4500", date: "2025-12-01" },
  { id: "3", name: "Robert Johnson", amount: "$4800", date: "2025-12-01" },
  { id: "4", name: "Emily Davis", amount: "$4700", date: "2025-12-01" },
];

const Shareholderpayrol: React.FC = () => {
  const [isMobile, setIsMobile] = useState(Dimensions.get("window").width < 768);
  const [mobileSidebar, setMobileSidebar] = useState(false);
  const [showNotifications, setShowNotifications] = useState(false);
  const notifAnim = useRef(new Animated.Value(300)).current;
  const [searchQuery, setSearchQuery] = useState("");
  const [filteredData, setFilteredData] = useState(dummyPayrollData);
  ...
  const handleSearch = () => {
    if (!searchQuery.trim()) setFilteredData(dummyPayrollData);
    else
      setFilteredData(
        dummyPayrollData.filter((item) =>
...
export default Shareholderpayrol;
